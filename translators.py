...
tokenizer = AutoTokenizer.from_pretrained("ai4bharat/indictrans2-indic-en-dist-200M", trust_remote_code=True)
...
def translate_to_en(text,spoken_lang):
    ...
    batch = ip.preprocess_batch(sentences, src_lang=lang_codes[spoken_lang], tgt_lang=lang_codes["english"], visualize=False) # set it to visualize=True to print a progress bar
    ...
    outputs = ip.postprocess_batch(outputs, lang_codes["english"])
    return outputs[0]
...
def translate_to_indic(text,trgt_lang):
    ...
    with torch.inference_mode():
        outputs = indic_trans_model.generate(**batch, num_beams=5, num_return_sequences=1, max_length=256)
